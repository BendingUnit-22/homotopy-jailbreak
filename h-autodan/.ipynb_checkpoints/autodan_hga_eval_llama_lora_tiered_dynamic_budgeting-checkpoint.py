...
from utils.opt_utils import get_score_autodan, autodan_sample_control, get_score_autodan_one_third_memory, get_score_autodan_med_memory
...
if __name__ == '__main__':
    ...
    if 'lora' in path.split('/')[-1] or 'lora' in path.split('/')[-2] or 'Lora' in path.split('/')[-1]:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            pretrained_model_name_or_path="/nobackup4/hooda/models/Llama-2-7b-chat-hf", 
            device_map=device, 
    ...
    if args.warm_start:
        ...
        # warm_start_data_path = "/nobackup3/divyam/data/break-lora/gcg_lora_results/llama2/autodan_Llama-2-7b-chat-hf_cold_start_advbench_0_500.json"
    ...
    assert len(budget_from_prev_ckpt)==len(harmful_data.goal[args.start:args.end])
    if args.warm_start:
    ...
    for i, (g, t) in tqdm(enumerate(dataset), total=len(harmful_data.goal[args.start:args.end])):
        ...
        for j in tqdm(range(num_steps + budget_from_prev_ckpt[i]), total=num_steps + budget_from_prev_ckpt[i]):
            with torch.no_grad():
                ...
                losses = get_score_autodan_med_memory( # Reduce memory usage.
                    tokenizer=tokenizer,
