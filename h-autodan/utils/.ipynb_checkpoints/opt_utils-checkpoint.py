...
def replace_with_synonyms(sentence, num=10):
    T = {"llama2", "meta", "vicuna", "lmsys", "guanaco", "theblokeai", "wizardlm", "mpt-chat",
         "mosaicml", "mpt-instruct", "falcon", "tii", "chatgpt", "modelkeeper", "prompt", "mistral", "zephyr",}
...
def get_score_autodan(tokenizer, conv_template, instruction, target, model, device, template_name, test_controls=None, crit=None, dis=None):
    ...
    for ids in input_ids_list:
        ...
        if template_name=='mistral' or template_name=='zephyr':
            padded_ids = torch.cat([torch.full((pad_length,), pad_tok, device=device), ids], dim=0)
        else:                                       
...
def get_score_autodan_med_memory(tokenizer, conv_template, instruction, target, model, device, template_name, test_controls=None, crit=None, dis=None):
    # Convert all test_controls to token ids and find the max length
    input_ids_list = []
    target_slices = []
    for item in test_controls:
    ...
    k = 2
    input_size = len(padded_input_ids_list)
    temp_logits = []
    for i in range(k):
        input_ids_tensor = torch.stack(padded_input_ids_list[i*input_size//k:(i+1)*input_size//k], dim=0)
        attn_mask = (input_ids_tensor != pad_tok).type(input_ids_tensor.dtype)
        logits = forward(model=model, input_ids=input_ids_tensor, attention_mask=attn_mask, batch_size=len(test_controls))
        temp_logits.append(logits)
        
    logits = torch.cat(temp_logits, dim=0)
    input_ids_tensor = torch.stack(padded_input_ids_list, dim=0)
...
def get_score_autodan_one_third_memory(tokenizer, conv_template, instruction, target, model, device, template_name, test_controls=None, crit=None, dis=None):
    # Convert all test_controls to token ids and find the max length
    input_ids_list = []
    target_slices = []
    for item in test_controls:
...
def get_score_autodan_one_fourth_memory(tokenizer, conv_template, instruction, target, model, device, template_name, test_controls=None, crit=None, dis=None):
    # Convert all test_controls to token ids and find the max length
    input_ids_list = []
    target_slices = []
    for item in test_controls:
...
def get_score_autodan_low_memory(tokenizer, conv_template, instruction, target, model, device, template_name, test_controls=None,
                                 crit=None):
    losses = []
    for item in test_controls:
        suffix_manager = autodan_SuffixManager(tokenizer=tokenizer,
